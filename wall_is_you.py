...
pm.init()
home_music_theme = pm.Sound("Media/Music/Home_screen_OST.mp3")
fight_music_theme_1 = pm.Sound("Media/Music/Theme1.mp3")
fight_music_theme_2 = pm.Sound("Media/Music/Theme2.mp3")
fight_music_theme_3 = pm.Sound("Media/Music/Theme3.mp3")
...
def window(
    ...
    leave, main_menu, already_drawn, music, alreadySaved = (
        ...
        False,
        False,
        False,  # variables pour gérer les événements
        False,
    )
    play_obj.stop()
    play_obj = fight_theme.play(loops=-1)
    tombs = []
    tombs.clear()
    win = fin_partie(goku, ennemis)  # verifie si le joueur a gagné ou perdu
    while (
        not leave and not main_menu and win == 0
    ):  # tant que le joueur n'a pas quitté ou gagné ou perdu
        leave = False
        ball = None
        ...
        while True and win == 0:
            if not already_drawn:  # si le labyrinthe n'est pas encore dessiné
                directions = [
                ...
                already_drawn = True  # le labyrinthe est dessiné
            ...
            if ty == "Quitte":
                ...
                if dragon_balls > 0 and not ball:
                    if goku["position"] != (i, j):
                        ...
                        if not on_ennemi:
                            ...
                            already_drawn = False  # le labyrinthe n'est plus dessiné
                ...
                if f.touche(ev) == "space":
                    ...
                    (
                        ...
                        goku,
                        ennemis,
                        dragon_balls,  # reinitialise le labyrinthe
                        nuage,
                        senzu,
                        map,
                        leave,
                    ...
                    f.mise_a_jour()  # affiche le menu pause
                    ...
                    save(
                        ...
                        goku,
                        ennemis,
                        dragon_balls,  # sauvegarde le labyrinthe
                        nuage,
                        senzu,
                        map,
                        alreadySaved,
                    ...
                    alreadySaved = True  # le labyrinthe est sauvegardé
    ...
    if win == 1:  # si le joueur a gagné
        ...
        f.mise_a_jour()
        f.attend_ev()
    elif win == -1:  # si le joueur a perdu
        f.image(
            rapport(f.largeur_fenetre(), f.largeur_fenetre() / 2),
            rapport(f.hauteur_fenetre(), f.hauteur_fenetre() / 2),
    ...
    return (
        leave,
        play_obj,
    )  # retourne si le joueur a quitté ou non et l'objet de la musique
...
def goku_intention(lab, ennemis, ball, directions, deque):
    ...
    visited = set()  # ensemble des cases visitées
    the_goal = None  # la case à atteindre
    first_iteration = (
        True  # variable pour savoir si on est au premier tour de la boucle
    )
    while len(deque) != 0:
        i, j, old_i, old_j = deque.popleft()
        if first_iteration:
            original_i, original_j = i, j  # position de goku
        ...
        if not check_ennemi(ennemis, (i, j)) and not check_dragon_balls(ball, (i, j)):
            for new_i, new_j in [(i, j - 1), (i + 1, j), (i - 1, j), (i, j + 1)]:
                if (
                    ...
                    deque.append(
                        (new_i, new_j, i, j)
                    )  # ajoute les voisins de la case courante dans la file

    if (
        ball is None or directions[ball["position"][0]][ball["position"][1]] is None
    ):  # si le tresor n'est pas accessible
    ...
    if the_goal is not None:
        m, n = the_goal
        way = [the_goal]
        while (m, n) != (original_i, original_j):  # remonte le chemin
    ...
    with open(fichier, "r", encoding="utf_8") as file:
        ...
        for ligne in lines:
            ...
            if (
                ...
                and ligne[0] != "S"
                and ligne[0] != "M"
            ):  # si la ligne est une ligne de labyrinthe
                for char in ligne:
                    if char == "═":
                ...
                if L:
                    ...
                    lab.append(L)
            elif ligne[0] == "A":  # si la ligne est la ligne de goku
                if len(datas) == 3:
            ...
            elif ligne[0] == "D":  # si la ligne est la ligne d'un ennemi
                if len(datas) == 3:
                    if (0 <= datas[0] < len(lab)) and (0 <= datas[1] < first_line):
                        ennemis.append(
                            {
            ...
            elif ligne[0] == "T":  # si la ligne est la ligne du nombre de dragon balls
                if len(datas) == 1:
...
def save(
    ...
    if not load:  # si la partie n'est pas une partie chargée
        ...
        if alreadySaved:
            count -= 1
        with open(
            "saved/saved_files.txt", "a+"
        ) as file:  # ajoute le fichier de sauvegarde dans la liste des fichiers de sauvegarde
    ...
    with open(fich, "w") as file:  # ecrit dans le fichier de sauvegarde
        for row in lab:
            for room in row:
...
def click_game_options(oneTrySolve):  # menu des options de jeu
...
def menu(play_obj):  # menu principal
    ...
    fight_theme = fight_music_theme_1
    music = True
    play_obj = home_music_theme.play(loops=-1)
    fichier = None
    ...
    while True:
        ...
        if ty1 == "Quitte":
            ...
            if rapport(width, 22) <= x <= rapport(width, 150) and rapport(
                ...
                while True:
                    ...
                    if ty4 == "Quitte":
                        ...
                        if rapport(width, 175) <= x <= rapport(width, 425) and rapport(
                            ...
                            while True:
                                ...
                                if ty1 == "Quitte":
                                    ...
                                    if rapport(width, 152) <= x <= rapport(
                                        ...
                                        created = True  # si le joueur a créé une map
                                        leave = True
                                        break
                                        ...
                                        menu_to_choose_map(
                                            width, length
                                        )  # si le joueur a choisi une map par défaut
                                        f.mise_a_jour()
                                        load = False
                                        while True:
                                            width, length = (
                            ...
                            saved_files = []
                            with open("saved/saved_files.txt", "r") as file:  # Continue
                                lines = file.readlines()
                                for line in lines:
                                    fichier = line.strip()
                ...
                while True:
                    ...
                    if ty2 == "Quitte":
                        ...
                        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                            ...
                            music = True
                            play_obj.stop()
                            play_obj = fight_music_theme_1.play(loops=-1)
                            fight_theme = fight_music_theme_1
                            ...
                            music = True
                            play_obj.stop()
                            play_obj = fight_music_theme_2.play(loops=-1)
                            fight_theme = fight_music_theme_2
                        elif rapport(width, 525) <= x <= rapport(
                            width, 584
                        ) and rapport(length, 384) <= y <= rapport(length, 436):
                            music = True
                            play_obj.stop()
                            play_obj = fight_music_theme_3.play(loops=-1)
                            fight_theme = fight_music_theme_3
                ...
                quit = True
                break
        elif ty1 == "Redimension":  # si la fenêtre est redimensionnée
            f.efface_tout()
            width = f.largeur_fenetre()  # on redessine le menu
            length = f.hauteur_fenetre()
            start_menu(width, length)
        if leave or quit:
            break
    ...
    return (
        ...
        load,
        fichier,
        quit,  # on renvoie les données du menu
        created,
        current_player,
        fight_theme,
        play_obj,
...
if __name__ == "__main__":
    f.cree_fenetre(600, 600, redimension=True, frequence=1000)  # on crée la fenêtre
    play_obj = home_music_theme.play(loops=-1)  # on lance la musique du menu
    play_obj.stop()
    main_menu = False  # variable qui permet de quitter vers le menu
    leave = False  # variable qui permet de quitter le jeu
    music = False  # variable qui permet de couper la musique
    dragon_balls = 3  # nombre de dragon balls
    while not leave:  # boucle principale du jeu
        (
            map,
            character,
            music,
            ...
            load,
            fichier,
            quit,  # on récupére les données du menu
            created,
            current_player,
            fight_theme,
            play_obj,
        ...
        if not quit:  # si le joueur ne quitte pas le jeu
            if load:  # si le joueur veut charger une partie
                (
                    lab,
                    goku,
                    ennemis,
                ...
                if created:  # si le joueur veut créer un labyrinthe
                    f.image(
                        rapport(f.largeur_fenetre(), 300),
                        rapport(f.hauteur_fenetre(), 300),
                        "Media/background_images/instructions.gif",
                        largeur=int(f.largeur_fenetre() * 1.01),
                        hauteur=int(f.hauteur_fenetre() * 1.01),
                        ancrage="center",
                    )
                    ...
                    try:
                        (
                            ...
                            goku,
                            ennemis,
                            nuage,  # on récupére les données du labyrinthe
                            senzu,
                            map,
                            quit,
                            leave,
                        ...
                        leave = True  # si le joueur quitte l'éditeur
                        continue  # on revient au menu principal

                    if leave:
                        leave = False
                        continue  # on revient au menu principal
                    if quit:  # si le joueur quitte l'éditeur
                        leave = True

                else:  # si le joueur veut jouer sur un labyrinthe existant
                    (
                        lab,
                        goku,
                        ennemis,
                        dragon_balls,  # on récupére les données du labyrinthe
                        nuage,
                        senzu,
                        map,
                        leave,
                    ) = labyrinth_generator(
                        f"Media/maps/map{map}.txt"
            ...
            leave, play_obj = window(  # on lance le jeu
                lab,
                goku,
                current_player,
                ennemis,
            ...
            leave = True  # si le joueur quitte le jeu
            f.ferme_fenetre()
